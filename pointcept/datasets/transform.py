...
@TRANSFORMS.register_module()
class RandomDistanceDropout(object):
    def __init__(self, dropout_ratio=1, dropout_application_ratio=1, mode='linear', steepness=10, midpoint=0.5, maxdist=None, reverse=False, max_ratio=None, min_ratio=None):
        self.dropout_ratio = dropout_ratio
        self.dropout_application_ratio = dropout_application_ratio
        self.mode = mode
    ...
    def __call__(self, data_dict):
        if random.random() < self.dropout_application_ratio:
            distances = np.linalg.norm(data_dict["coord"], axis=1)
            ...
            if self.mode == 'linear':
                dropout_probs = normalized_distances * self.dropout_ratio
            elif self.mode == 'sigmoid':
                dropout_probs = 1 / (1 + np.exp(-self.steepness * (normalized_distances - self.midpoint)))
                dropout_probs = dropout_probs * self.dropout_ratio
            else:
                pass
            if self.reverse:
                dropout_probs = 1 - dropout_probs
            if self.max_ratio:
                dropout_probs = np.clip(dropout_probs, 0, self.max_ratio)
            if self.min_ratio:
                dropout_probs = np.clip(dropout_probs, self.min_ratio, 1)
            random_vals = np.random.rand(data_dict["coord"].shape[0])
            idx = random_vals > dropout_probs
...
class GridSample(object):
    def __init__(
        ...
        self.if_not_index_operator = if_not_index_operator
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "train":  # train mode
            ...
            if not self.if_not_index_operator:
...
class AddNoise(object):
    ...
    def __call__(self, data_dict):
        ...
        if rand < self.p:
            ...
            data_dict['coord'] = np.concatenate((coord, noise), axis=0)
            if "strength" in data_dict.keys():
                strength = data_dict["strength"]
                imin, imax = strength[:, 0].min(), strength[:, 0].max()
                noise_i = np.random.normal(loc=(imin + imax) / 2, scale=0.5, size=noise_num).astype(np.float32).reshape(
                    -1, 1)
                data_dict['strength'] = np.concatenate((strength, noise_i), axis=0)
            if "segment" in data_dict.keys():
                label = data_dict["segment"]
                data_dict['segment'] = np.concatenate(
                    (label, np.ones(noise.shape[0], dtype=np.int64) * self.ignore_index), axis=0)
        return data_dict
